...
The instanciation of an object of class `Tama` from R runs a tamagotchi and provide an R interface for it. The emulation is real-time. A single tamagotchi can be alive on a given R session: instanciating several `Tama`'s will crash them. If you need several pets, run several R sessions.
